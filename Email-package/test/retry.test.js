...
describe("RetryHandler", () => {
  ...
  beforeEach(() => {
    const mockSendProvider1 = jest.fn()
      .mockRejectedValueOnce(new Error("temporary error"))
      .mockResolvedValueOnce({ messageId: "msg-1", envelopeTime: 100, messageTime: 200 });

    const mockSendProvider2 = jest.fn()
      .mockResolvedValue({ messageId: "msg-2", envelopeTime: 150, messageTime: 250 });
  ...
  test("calls onError when retry fails and switches provider", async () => {
    providers[0].send = jest.fn()
      .mockRejectedValueOnce(new Error("temporary error"))
      .mockRejectedValueOnce(new Error("temporary error again"));
  ...
  test("switches to next provider if first provider fails fatally", async () => {
    providers[0].send = jest.fn().mockRejectedValue(new Error("quota exceeded"));
    const hooks = {
      onSendStart: jest.fn(),
      onProviderSwitch: jest.fn(),
    ...
    expect(providers[1].send).toHaveBeenCalledTimes(1);
    expect(hooks.onProviderSwitch).toHaveBeenCalled();
    expect(hooks.onFinalSuccess).toHaveBeenCalled();
